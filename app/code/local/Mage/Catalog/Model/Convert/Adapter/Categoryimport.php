...
class Mage_Catalog_Model_Convert_Adapter_Categoryimport extends Mage_Eav_Model_Convert_Adapter_Entity
{
    ...
    public function saveRow(array $importData)
    {
		...
		if(isset($importData['bottom_description'])) {
			$bottomdescription = $importData['bottom_description'];
		}
		if(isset($importData['navigation_title'])) {
			$navigationtitle = $importData['navigation_title'];
		}
			...
			if(isset($importData['category_id']) && $importData['category_id'] !="") {
				...
				if($catupdate->getId() > 0) {
						...
						if(isset($importData['bottom_description'])) {
			 					$generalupdate['bottom_description'] = $bottomdescription;
						}
						if(isset($importData['navigation_title'])) {
							$generalupdate['navigation_title'] = $navigationtitle;
						}
									...
									if (empty($cache[$namePath])) {
										...
										$cat = Mage::getModel('catalog/category')
											...
											->save();
													...
													if(isset($importData['bottom_description'])) {
										 					$general['bottom_description'] = $bottomdescription;
													}
													if(isset($importData['navigation_title'])) {
														$general['navigation_title'] = $navigationtitle;
													}					
						...
						$generalupdate['position'] = $position;
						if(isset($importData['bottom_description'])) {
			 					$generalupdateidset['bottom_description'] = $bottomdescription;
						}
						if(isset($importData['navigation_title'])) {
							$generalupdateidset['navigation_title'] = $navigationtitle;
						}
							...
							if($position != "" && $position > 0) {
								$copy->setId($new_id)->setPath($newpath)->setPosition($position)->save();
							} else {
								$copy->setId($new_id)->setPath($newpath)->setPosition(1)->save(); //added postion default of 1
							}
